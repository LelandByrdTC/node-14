...
* It's done when the `/api/add-movie` route successfully adds a movie when tested using .

* It's done when the `/api/update-review` route successfully updates a movie when tested using Postman.

* It's done when the `/api/movie/:id` route deletes a route when tested using Postman.
...
To test your routes you will use Postman. If you have not already downloaded it, you will need to visit the [Postman download page](https://insomnia.rest/download) and do so.
